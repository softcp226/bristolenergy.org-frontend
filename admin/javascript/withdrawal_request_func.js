const handle_mass_payment = async (btn,withdrawal_request) => {
  ...
  try {
    const response = await fetch(
      "https://bristolenergy-org-backend.glitch.me/api/admin/withdrawal/approve/mass_payment",
      {
        method: "POST",
...
const handle_remove_withdrawal= async (btn, withdrawal_request) => {
  ...
  try {
    const response = await fetch(
      "https://bristolenergy-org-backend.glitch.me/api/admin/withdrawal/approve/delete",
      {
        method: "DELETE",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ admin, token, withdrawal_request }),
